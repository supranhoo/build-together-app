...
- **Spec fields MUST be visible at-a-glance in both list views (2026-04-27).** The Item Master list MUST show a `Specs` column rendering each item's stored specs as compact `key: value` chips (cap 6 visible, with `+N` overflow). The Spec Templates list MUST show field-label chips inline (with unit folded in, cap 8 visible, `+N` overflow) and provide an expand toggle that reveals the full per-field metadata table (Key · Label · Unit · Required · Numeric · Min · Max). Both views MUST source from the same pure helpers in `src/lib/spec-summary.ts` so formatting stays consistent.
