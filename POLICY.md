...
- **Item Master specs are per-item, not derived from a master spec template.** Per workspace decision (2026-04-26), `materials.specs` remains a free-form `Record<string, unknown>` shared across all consumers. The Item Master form provides a structured rows editor (key, value, unit, required, numeric, min, max) for ergonomics, but enforced constraints (`required`, `numeric`, range) are session-local UI rules — they are NOT persisted alongside each spec value because no master `spec_templates` table exists. Future centralization of allowed specs per material nature must go through a documented schema-migration phase and MUST NOT be inferred from existing item rows.
