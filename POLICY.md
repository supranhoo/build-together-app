...
- **Bulk upload (CSV)**: Item Master accepts CSV bulk upload. The importer MUST go through the same `upsertMasterItem` SSOT used by the single-item dialog — it MUST NOT bypass RLS, MUST NOT skip the `audit_logs` write, and MUST NOT introduce a parallel insert path. Per-row failures are collected and reported to the user; one bad row never aborts the batch. **Import/Template header (canonical, code-free)**: `name,type,group_name,subgroup,uom,std_cost,<one column per FIXED_SPEC_COLUMNS entry: Mn,Moisture,Fe,SiO2,CaO,Al2O3,MgO,P,S,FC,VM,Ash,Si,Size>,is_active`. Codes are system-assigned (`<TYPE>-<GROUP>-<NNNN>`) — the importer MUST reject any uploaded file that contains a `code` column. **Export header** prepends a read-only `code` column for operator reference (2026-05-22): operators can use the exported CSV outside the app, but re-uploading it via Bulk upload is rejected by the `code`-column guard. `type` MUST validate against the `MATERIAL_TYPES` enum — never accept free-text values. Custom/non-standard spec keys are NOT supported via CSV; they must be added in the Item Master editor.
- Pin sort_order is user-controlled. Reordering MUST NOT trigger any KPI recomputation — pins are display metadata only.
...
- **FAD Product Name (2026-05-29):** the Product Name field on the FAD heat-entry header MUST be chosen from the `fad.finished_good` picker context (default: `material_type=FG`, no group constraint, `allow_unmapped=false`). Free-text entry has been removed to enforce SSOT — only items in Item Master can be produced. Admins assign new Ferro-Alloy products by tagging items `type=FG` in Item Master (any Ferro-Alloy sub-group such as *High Carbon Ferro Manganese*, *Silico Manganese*, etc.), or by narrowing the context per workspace. **Display rule:** this picker shows the item **name only** (codes are hidden) with a live **current-stock preview** (aggregated across locations) beside each option so operators pick by familiar product name and see availability at a glance.
...
- **User lifecycle** — *Delete* (deactivate via approval) MUST go through approvals: soft-delete via `profiles.is_active=false` + revoke roles + deactivate PC assignments. Self-deletion is blocked in UI. *Create* is a DIRECT admin action: admin supplies the initial password via `admin-create-user` (no approval). *Reset password*, *Activate/Deactivate toggle*, and *Change login email* are also direct admin actions (`admin-reset-password`, `admin-set-user-active`, `admin-change-user-email`). All five actions are audit-logged server-side. Admins cannot change their own email through this flow (self-target blocked) to prevent lock-out from typos. `profiles.email` mirrors `auth.users.email` and must only be mutated through these admin functions. User-management client flows must surface backend validation/auth errors returned by these functions rather than replacing them with generic transport errors. See "User Management (2026-05-21)" below.
...
- Format: `<TYPE>-<GROUP>-<NNNNN>` (e.g. `RM-ORE-00001`), 5-digit zero-padded suffix.
