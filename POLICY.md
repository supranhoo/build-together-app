...
- 2026-04-27 (Specs as fixed columns): replaced the chip cell on the Item Master list with seven dedicated columns (Mn / Fe / Si / P / S / Moisture / Size); same column block added to the Spec Templates list showing the enforced range per cell. Single named constant `FIXED_SPEC_COLUMNS` + alias-aware `getSpecValue` in `src/lib/spec-columns.ts`; 5 new unit tests.
