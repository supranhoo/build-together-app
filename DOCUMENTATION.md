...
- **`fad.finished_good`** (2026-05-29): drives the **Product Name** dropdown on the FAD heat-entry header (`PortalProductionFAD.tsx`). Default filter `material_type=FG` only (no group constraint — items live under sub-groups like *High Carbon Ferro Manganese*, *Silico Manganese*, *Ferro Silicon*; constraining by group would hide all of them). `allow_unmapped=false`. Selecting an item populates `productName` (still saved as text in `heat_logs.product`) and prefills **Typical Grade** from `item.specs.typicalGrade` when present and the field is empty. **RCA (2026-05-29):** initial seed used `group_name='Ferro Alloys'` but no item carries that literal group → dropdown was empty for live workspaces; group filter relaxed to fix. **Display (2026-05-29):** this picker uses `MaterialPicker displayMode="name-only"` with a `stockByMaterial` map (sum of `inventory_ledger` across locations) so each option shows only the item name plus a live "Stock: <qty> <uom>" preview — hides the noisy `FG-HIGHCARBONFERROMANGANESE-…` codes that otherwise look identical when truncated.
...
1. **Auto-generated item Code** — On *new* items, Code is read-only and generated as `<TYPE>-<GROUP>-<NNNNN>` (zero-padded, 5 digits as of 2026-05-29; previously 4) by `nextItemCode()` (`src/lib/master-items-code.ts`). Sequence is derived client-side at dialog interaction by scanning existing `materials.code` values that share the same prefix and incrementing the highest numeric tail (legacy 4-digit codes continue to be honoured — `RM-ORE-0009` → next `RM-ORE-00010`). Edit mode keeps the existing code editable so admins can correct legacy rows. CSV bulk upload still accepts user-supplied codes (unchanged).
